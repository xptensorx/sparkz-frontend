export default function SparkzLogo({ size = 'md', variant = 'dark', iconOnly = false }) {
  ...
  const textColor = variant === 'light' ? 'text-white' : 'text-brand-indigo';

  return (
    <div className={`flex items-center gap-2.5 ${iconOnly ? 'justify-center' : ''}`}>
      <div className={`${iconSize} bg-brand-gold rounded-xl flex items-center justify-center flex-shrink-0`}>
